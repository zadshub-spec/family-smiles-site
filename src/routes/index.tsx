    ...
    <div className="min-h-screen overflow-x-hidden bg-background">
      <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <Button asChild className="hidden h-11 rounded-xl px-5 font-semibold lg:inline-flex"><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer"><CalendarDays /> Book an Appointment</a></Button>
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-14 sm:py-20 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-24">
            <div className="max-w-xl">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg" className="h-14 rounded-xl px-7 text-base font-bold shadow-lg shadow-primary/20"><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer"><CalendarDays /> Book an Appointment</a></Button>
                <Button asChild size="lg" variant="outline" className="h-14 rounded-xl border-primary/30 px-7 text-base font-bold text-primary"><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer"><MessageCircleHeart /> WhatsApp +1 876-774-6645</a></Button>
              </div>
        ...
        <section id="services" className="scroll-mt-20 bg-muted py-20 sm:py-28">
          <div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mx-auto max-w-2xl text-center"><p className="text-sm font-bold uppercase text-primary">Our Services</p><h2 className="mt-4 text-4xl font-bold sm:text-5xl">Dental Care for Every Smile</h2><p className="mt-5 text-lg leading-8 text-muted-foreground">Explore our general service categories and contact us to discuss your dental care needs.</p></div>
            <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map(({icon: Icon,title,description}) => <article key={title} className="group rounded-2xl border border-border bg-card p-7 transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/5"><span className="grid size-13 place-items-center rounded-2xl bg-secondary text-primary"><Icon className="size-6" /></span><h3 className="mt-6 text-xl font-bold">{title}</h3><p className="mt-3 min-h-12 leading-6 text-muted-foreground">{description}</p><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center gap-1 text-sm font-bold text-primary">Learn More <ChevronRight className="size-4 transition-transform group-hover:translate-x-1" /></a></article>)}</div>
          </div>
        </section>

        <section id="why-us" className="scroll-mt-20 py-20 sm:py-28"><div className="mx-auto grid max-w-7xl items-center gap-12 px-5 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:px-8"><img src={careImage} alt="A dental professional listening attentively to a patient" width={1104} height={1200} loading="lazy" className="aspect-[4/4.35] max-h-[610px] w-full rounded-[2rem] object-cover" /><div><p className="text-sm font-bold uppercase text-primary">Why Choose Siri Family Dental?</p><h2 className="mt-4 text-4xl font-bold leading-tight sm:text-5xl">A Comfortable Place for Your Dental Care</h2><div className="mt-9 grid gap-6 sm:grid-cols-2">{[{ icon: Smile, title: "Family-Friendly Environment", text: "A welcoming experience for patients of different ages." }, { icon: MapPin, title: "Convenient Location", text: "Located on Fernleigh Avenue in May Pen." }, { icon: HeartHandshake, title: "Patient-Focused Approach", text: "We aim to help patients feel comfortable and listened to." }, { icon: MessageCircleHeart, title: "Easy Appointment Contact", text: "Patients can message the clinic directly on WhatsApp." }].map(({ icon: FeatureIcon, title, text }) => <div key={title} className="flex gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-accent text-accent-foreground"><FeatureIcon className="size-5" /></span><div><h3 className="font-bold">{title}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{text}</p></div></div>)}</div><Button asChild size="lg" className="mt-9 h-13 rounded-xl px-7 text-base font-bold"><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer"><CalendarDays /> Book an Appointment</a></Button></div></div></section>

        <section id="faq" className="scroll-mt-20 bg-secondary/60 py-16"><div className="mx-auto grid max-w-7xl gap-8 px-5 md:grid-cols-3 lg:px-8"><div><p className="text-sm font-bold uppercase text-primary">Helpful details</p><h2 className="mt-3 text-3xl font-bold">Plan your visit</h2></div><div><h3 className="font-bold">When are you open?</h3><p className="mt-2 leading-7 text-muted-foreground">Monday to Saturday, 9:00 AM – 5:00 PM. Closed Sunday.</p></div><div><h3 className="font-bold">How can I book?</h3><p className="mt-2 leading-7 text-muted-foreground">Message us on WhatsApp at <a className="font-semibold text-primary" href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer">+1 876-774-6645</a> to arrange an appointment.</p></div></div></section>

        <section id="contact" className="scroll-mt-20 bg-primary py-16 text-primary-foreground"><div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-5 lg:flex-row lg:items-center lg:px-8"><div><p className="text-sm font-bold uppercase opacity-80">Ready when you are</p><h2 className="mt-3 text-3xl font-bold sm:text-4xl">Let&apos;s take care of your smile.</h2><p className="mt-3 max-w-2xl opacity-85">Message Siri Family Dental on WhatsApp to book an appointment or ask about your dental care needs.</p></div><Button asChild size="lg" variant="secondary" className="h-14 shrink-0 rounded-xl px-7 text-base font-bold"><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer"><MessageCircleHeart /> WhatsApp +1 876-774-6645</a></Button></div></section>
      </main>

      <footer className="bg-foreground py-14 text-background"><div className="mx-auto grid max-w-7xl gap-10 px-5 md:grid-cols-3 lg:px-8"><div><div className="flex items-center gap-3"><img src={logoAsset.url} alt="" width={56} height={56} loading="lazy" className="size-14 rounded-full border border-background/20 object-cover" /><strong className="font-display text-xl">Siri Family Dental</strong></div><p className="mt-4 max-w-xs text-sm leading-6 opacity-70">Comfortable and welcoming dental care for you and your family in May Pen.</p></div><div><h2 className="font-bold">Visit us</h2><address className="mt-4 text-sm not-italic leading-7 opacity-70">National Bank, Fernleigh Avenue Upstairs<br />45 A Main Street<br />May Pen, Jamaica</address></div><div><h2 className="font-bold">Contact & hours</h2><a href="https://wa.me/18767746645" target="_blank" rel="noopener noreferrer" className="mt-4 block text-sm font-semibold text-accent">+1 876-774-6645 (WhatsApp)</a><p className="mt-2 text-sm leading-7 opacity-70">Monday–Saturday: 9:00 AM–5:00 PM<br />Sunday: Closed</p></div></div><div className="mx-auto mt-12 max-w-7xl border-t border-background/15 px-5 pt-6 text-xs opacity-55 lg:px-8">© {new Date().getFullYear()} Siri Family Dental. May Pen, Jamaica.</div></footer>
    </div>
  );
}
